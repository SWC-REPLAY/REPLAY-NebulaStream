...
std::string StoreRegistry::registerDefaultStore(const std::string& storeName, const Schema& schema, const std::string& schemaText)
{
    ...
    levels.push_back(StoreLevel{
        .store = makeStore<MemoryStore>(schema),
        .policy = FlushPolicy{.type = FlushPolicy::Type::SIZE_THRESHOLD}});
    levels.push_back(StoreLevel{
        .store = makeStore<FileStore>(FileStore::Config{.storeName = storeName, .filePath = filePath, .schemaText = schemaText}, schema),
        .policy = FlushPolicy{}});
