...
TEST_F(SystestParserValidTestFileTest, ValidTestFile)
{
    ...
    parser.registerOnQueryCallback([&](const std::string&, SystestQueryId, bool, bool) { queryCallbackCalled = true; });
...
TEST_F(SystestParserValidTestFileTest, Nullable1TestFile)
{
    ...
    parser.registerOnQueryCallback(
        [&queryCallbackCalled, &expectedQueries](const std::string& query, const SystestQueryId currentQueryIdInTest, bool, bool)
        {
...
TEST_F(SystestParserValidTestFileTest, FilterTestFile)
{
    ...
    parser.registerOnQueryCallback(
        [&](const std::string& query, const SystestQueryId currentQueryIdInTest, bool, bool)
        {
            queryCallbackCalled = true;
            /// Query numbers start at QueryId::INITIAL, which is 1
            ASSERT_LT(currentQueryIdInTest.getRawValue(), expectedQueries.size() + 1);
...
TEST_F(SystestParserValidTestFileTest, ErrorExpectationTest)
{
    ...
    SystestParser parser{};
    parser.registerOnQueryCallback(
        [&queryCallbackCalled, &expectQuery](const std::string& query, SystestQueryId, bool, bool)
        {
